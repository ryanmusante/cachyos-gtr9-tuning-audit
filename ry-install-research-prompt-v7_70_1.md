...
Hard `--verify` asserts (mismatch → exit 1): scaling_driver, scaling_governor=powersave, EPP=balance_performance, amd_pstate status/prefcore, boost, clocksource, GPU power_dpm=high, regdom (`/etc/iw-regdomain`; the second regdom file is removed — do not verify `conf.d/wireless-regdom`), nftables `nd-neighbor-solicit` present. Warn-level only: ZRAM, live nftables NDP cross-check, iwd runtime state. No boot-time, THP, or KSM checks (removed). No `ttm.*`, drirc, or `radv_enable_unified_heap_on_apu` globals exist (removed) — do not verify them.
...
- Establish the minimum mainline and CachyOS kernel that fully enables gfx1151 display, amdgpu, amd_pstate-EPP, and ACP; check the installed kernel; assess whether a hard floor should be reinstated (the r8169 landing version, §11, is the strongest concrete floor).
- Confirm the soft Mesa 26.0 floor matches current RADV guidance (raised 25.3 → 26.0 in v7.70.0); enumerate open gfx1151 RADV issues and state whether 26.0 is the right threshold.
...
- CachyOS repo tier: does `x86-64-v4`/znver (AVX-512) apply and benefit this build over v3?
...
- **iommu=pt:** confirm AMD-Vi is left enabled by default, so dropping explicit `amd_iommu=on` is genuinely redundant and not a silent revert to off; quantify the gaming/compute cost of `pt` vs no param; confirm it is the recommended posture for ROCm + USB4 isolation.
- **clearcpuid=514 (UMIP off):** establish the real benefit (avoiding umip_printk stutter under anti-cheat/Wine) vs the security cost (descriptor-table leak, kernel taint). Confirm whether current Proton/EAC/BattlEye actually trip UMIP emulation on Zen 5. Recommend dropping if no stutter is observed.
- **amd_pstate=active:** confirm recommended on Zen 5 and its interaction with the powersave governor + balance_performance EPP (§6).
...
- Zero amdgpu/ttm module params: confirm the hands-off GPU-param posture is correct (GTT via kernel default §5, GPU clock via udev §6).
...
- zstd -1 -T0: confirm the optimal level for NVMe; quantify any higher-level boot-time win (no boot-time target is pinned).
...
- **RADV unified heap (drirc removed):** confirm current RADV on gfx1151 reports uma:1 and treats the heap as unified without the override (via vulkaninfo heap flags / Mesa source). If not, flag a regression.
- **GTT sizing (ttm removed):** confirm the installed kernel auto-sizes GTT sensibly; establish the actual ceiling (~62 GiB). Is that right for both large-VRAM gaming and for ROCm/llama.cpp wanting ~96 GiB, or does removing the cap under-provision compute? If a workload needs >~62 GiB, determine the current correct mechanism (BIOS UMA carveout vs ttm.* vs sysfs) and whether the profile should reintroduce an opt-in knob. Confirm amdgpu.gttsize is still deprecated.
- **ntsync (observed, not deployed):** confirm it is the current Wine-sync mechanism (vs esync/fsync), which kernel mainlined it and which CachyOS kernel ships it, that Proton consumes /dev/ntsync and improves frametimes on 16C/32T, and assess whether the profile should take a position (autoload conf / verified env var) or leave it to CachyOS defaults. This is the largest unmanaged gaming surface.
- **PROTON_FSR4_RDNA3_UPGRADE removed:** confirm current Proton/Proton-CachyOS FSR4-upgrade support for RDNA 3.5 — is there now a verified env var? If yes, recommend (flag); if still unverified, the removal stands.
...
- The governor=powersave + EPP=balance_performance pairing is the EPP-honoring config under active mode (the performance governor pins max and ignores EPP). Do not flag powersave as a power regression or recommend reverting to performance without establishing that it would override the EPP hint.
- Confirm the active + powersave + balance_performance triple is the documented EPP-honoring max-perf config on Zen 5. State whether balance_performance (mid-bias) leaves boost residency on the table vs EPP=performance, and whether mid-bias is the better thermal/latency trade on a 140 W mini-PC.
- Confirm udev add|change pinning is robust vs one-shot; prefcore=enabled + boost=1 correct on Strix Halo.
- Mask power-profiles-daemon: confirm no needed platform_profile path is lost; does CachyOS expect ppd? Consider tuned.
- Mask ananicy-cpp: confirm net win for gaming.
- Mask modemmanager: confirm no cellular HW, so masking is loss-free.
- GPU clock-floor=high: (a) does forcing high vs auto help frametime/1%-lows on gfx1151, or just raise idle power? (b) on a shared 140 W package, does pinning GPU high steal CPU boost headroom and hurt CPU-bound titles, given EPP is now balance_performance? (c) high vs manual + pp_dpm_sclk vs auto? Flag and quantify thermal cost (--verify treats ≠high as drift).
...
- **zram trio:** confirm the running kernel accepts swappiness > 100; is 150 gratuitous on 128 GB RAM, or does it help large-alloc/LLM reclaim? Confirm page-cluster=0 (disable readahead) is the standard zram recommendation. Confirm vfs_cache_pressure=50 is a net win vs default 100 and does not starve the page cache. List which vendor 70-cachyos values the priority-95 file overrides and whether justified.
- zswap.enabled=0: confirm CachyOS uses zram (not zswap) by default — no double-compression conflict.
- NVMe scheduler none: confirm best practice vs mq-deadline/kyber on this kernel; evaluate nr_requests and read_ahead_kb.
- fstab noatime,lazytime,commit=10: confirm noatime and lazytime coexist; weigh commit=10 durability; confirm fstrim.timer over continuous discard.
...
- **NM backend wpa_supplicant (reverted from iwd):** verify current Arch/CachyOS guidance for MT7925/mt76 — is wpa_supplicant the more stable backend today, or has iwd matured to parity? (The profile has flipped 3×; weigh evidence, not momentum.) Confirm no leftover iwd/NM race and that the removed iwd/main.conf leaves no dangling reference. Confirm wifi.powersave=2 alone fully disables the mt76 software power-save latency issue under wpa_supplicant.
...
- tcp_notsent_lowat=16384, tcp_slow_start_after_idle=0: confirm the rationale holds.
...
- regdom US fixed: confirm correct max TX power and channel set for MT7925 on current cfg80211/wireless-regdb (the "3 dBm" readout is cosmetic — do not attribute to regdom). Flag if 6 GHz Wi-Fi 7 needs a specific AFC posture beyond a country code; note non-US deployers must hand-edit COUNTRY.
...
- Confirm nftables is enabled as the firewall and that ufw-flush-then-mask leaves no unfirewalled window.
...
- Validate the nftables shape is minimal-but-sufficient on a dual-10 GbE LAN; flag any port gaming/Proton/remote-play (Steam Remote Play, Sunshine/Moonlight) may need inbound.
...
- iommu=pt is now a net positive vs the prior amd_iommu=off; confirm AMD-Vi is genuinely enabled, record as a closed reduction.
- clearcpuid=514 is the new open reduction; quantify exposure vs the umip_printk stutter prevented (§3); list first in the security delta.
...
- Re-confirm the resolved claims: the MES 0x86 blob version carried by the installed linux-firmware + mkinitcpio-firmware, and the minimum kernel containing both r8169 commits (→ concrete kernel floor, §1). Give exact landing versions.
- MT7925 + ACP: check current upstream status (open/partial/landed). For MT7925, focus on panics/deauth (not the cosmetic readout); prefer a kernel/firmware floor over DKMS given the pacman-only posture. Note the wpa_supplicant revert (§8) is itself an MT7925-stability move — assess whether backend + kernel floor close the gap.
...
- Confirm each MangoHud directive is valid for current MangoHud (do not invent). With gpu_power and cpu_temp dropped, assess whether their removal loses the two most decision-relevant readouts on a 140 W shared-package APU — a possible TUNE-to-restore. Note toggle_hud removal means no Shift_R+F12 hide bind — assess whether a toggle should return. Establish the real minimum MangoHud + kernel for the remaining amdgpu sensors on gfx1151.
...
- Do not recommend reinstating deliberately removed values (amdgpu.ppfeaturemask, PROTON_FSR4_RDNA3_UPGRADE, `--country` flag, TTM/GTT cap, RADV drirc, MangoHud gpu_power/cpu_temp/toggle_hud) unless current upstream directly contradicts the removal rationale — and then flag, do not FIX.
- IOMMU special case: amd_iommu=off is itself removed; current is iommu=pt — treat changes under the reinstatement rule, record as a restored mitigation.
